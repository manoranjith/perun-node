...
func (a *payChAPIServer) StartWatchingLedgerChannel(srv pb.Watching_API_StartWatchingLedgerChannelServer) error {
	req, err := srv.Recv()
	if err != nil {
...
func signedStateFromProtoLedgerChReq(req *pb.StartWatchingLedgerChannelReq) (signedState *channel.SignedState, err error) {
	...
	for i := range sigs {
		sigs[i] = pwallet.Sig(req.Sigs[i])
	}
	return signedState, nil

}

func transactionFromProtoLedgerChReq(req *pb.StartWatchingLedgerChannelReq) (transaction *channel.Transaction, err error) {
	...
	for i := range transaction.Sigs {
		transaction.Sigs[i] = pwallet.Sig(req.Sigs[i])
	}
